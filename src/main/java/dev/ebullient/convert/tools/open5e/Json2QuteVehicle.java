...
import dev.ebullient.convert.tools.open5e.qute.QuteVehicle;
import dev.ebullient.convert.tools.open5e.qute.QuteVehicle.ShipAcHp;
import dev.ebullient.convert.tools.open5e.qute.QuteVehicle.ShipCrewCargoPace;
import dev.ebullient.convert.tools.open5e.qute.QuteVehicle.ShipSection;

public class Json2QuteVehicle extends Json2QuteCommon {

    final VehicleType vehicleType;

    Json2QuteVehicle(Open5eIndex index, Open5eIndexType type, JsonNode jsonNode) {
    ...
    protected Open5eQuteBase buildQuteResource() {
        ...
        List<String> text = getFluff(Open5eIndexType.vehicleFluff, "##", fluffImages);
    ...
    String vehicleSize(Tags tags) {
        ...
        if (Open5eFields.size.existsIn(rootNode)) {
    ...
    String vehicleSpeed() {
        if (vehicleType == VehicleType.INFWAR) {
            return speed(Open5eFields.speed.getFrom(rootNode));
        }
        return "";
    }
    ...
    private ShipCrewCargoPace shipCrewCargoPace() {
        ...
        if (vehicleType == VehicleType.SHIP) {
            ...
            shipAcHp = new ShipAcHp(vehicleType.name(),
                    ...
                    null);
            speedPace = speed(Open5eFields.speed.getFrom(rootNode));
