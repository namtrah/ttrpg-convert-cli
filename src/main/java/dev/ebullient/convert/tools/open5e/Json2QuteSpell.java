...
import dev.ebullient.convert.tools.open5e.qute.QuteSpell;

public class Json2QuteSpell extends Json2QuteCommon {

    final String decoratedName;

    Json2QuteSpell(Open5eIndex index, Open5eIndexType type, JsonNode jsonNode) {
    ...
    protected Open5eQuteBase buildQuteResource() {
        ...
        return new QuteSpell(sources,
                ...
                String.join(", ", classes),
                String.join("\n", text),
                getFluffImages(Open5eIndexType.spellFluff),
                tags);
    }
    ...
    Set<String> indexedSpellClasses(Tags tags) {
        ...
        return list.stream()
                ...
                .map(k -> {
                    Open5eSources sources = Open5eSources.findSources(k);
                    Open5eIndexType type = Open5eIndexType.getTypeFromKey(k);
                    if (type == Open5eIndexType.subclass) {
    ...
    Set<String> spellClasses(SpellSchool school, Tags tags) {
        ...
        classesNode.withArray("fromClassList").forEach(c -> {
            ...
            String finalKey = Open5eIndexType.classtype.createKey(className, classSource);
        ...
        classesNode.withArray("fromSubclass").forEach(s -> {
            ...
            String finalKey = Open5eIndexType.getSubclassKey(className.trim(), classSource.trim(), subclassName.trim(),
                    subclassSource.trim());
            if (index().isIncluded(finalKey)) {
        ...
        if (classes.contains("Wizard")) {
            if (school == SpellSchool.SchoolEnum.Abjuration || school == SpellSchool.SchoolEnum.Evocation) {
                String finalKey = Open5eIndexType.getSubclassKey("Fighter", "PHB", "Eldritch Knight", "PHB");
                if (index().isIncluded(finalKey)) {
            ...
            if (school == SpellSchool.SchoolEnum.Enchantment || school == SpellSchool.SchoolEnum.Illusion) {
                String finalKey = Open5eIndexType.getSubclassKey("Rogue", "PHB", "Arcane Trickster", "PHB");
                if (index().isIncluded(finalKey)) {
    ...
    private String getClass(Tags tags, String className, String classSource, String classKey) {
        ...
        return linkOrText(
                className,
                classKey,
                Open5eIndexType.classtype.getRelativePath(),
    ...
    private String getSubclass(Tags tags, String className, String classSource, String subclassName,
        ...
        return linkOrText(
                ...
                subclassKey,
                Open5eIndexType.classtype.getRelativePath(),
                Open5eQuteBase.getSubclassResource(subclassName, className, subclassSource));
    }
