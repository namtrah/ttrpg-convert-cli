...
import dev.ebullient.convert.tools.open5e.qute.QuteDeck;
import dev.ebullient.convert.tools.open5e.qute.QuteDeck.Card;

public class Json2QuteDeck extends Json2QuteCommon {

    Json2QuteDeck(Open5eIndex index, Open5eIndexType type, JsonNode jsonNode) {
    ...
    protected Open5eQuteBase buildQuteResource() {
        ...
        for (JsonNode cardRef : DeckFields.cards.iterateArrayFrom(rootNode)) {
            ...
            if (cardRef.isTextual()) {
                cardKey = Open5eIndexType.card.fromRawKey(cardRef.asText());
            } else if (cardRef.isObject()) {
                cardKey = Open5eIndexType.card.fromRawKey(DeckFields.uid.getTextOrThrow(cardRef));
            } else {
                cardKey = null;
            }
