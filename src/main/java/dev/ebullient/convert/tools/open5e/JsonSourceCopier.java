...
public class JsonSourceCopier implements JsonSource {
    ...
    static final List<String> _MERGE_REQUIRES_PRESERVE_BASE = List.of(
            ...
            "reprintedAs",
            "srd");
    static final Map<Open5eIndexType, List<String>> _MERGE_REQUIRES_PRESERVE = Map.of(
            Open5eIndexType.monster, List.of("legendaryGroup", "environment", "soundClip",
                    "altArt", "variant", "dragonCastingColor", "familiar"),
            Open5eIndexType.item, List.of("lootTables", "tier"),
            Open5eIndexType.itemGroup, List.of("lootTables", "tier"));
    ...
    static boolean mergePreserveKey(Open5eIndexType type, String key) {
    ...
    final Open5eIndex index;

    JsonSourceCopier(Open5eIndex index) {
        this.index = index;
    }
    ...
    JsonNode handleCopy(Open5eIndexType type, JsonNode copyTo) {
    ...
    public JsonNode mergeSubrace(JsonNode subraceNode, JsonNode raceNode) {
        ...
        if (Open5eFields.traitTags.existsIn(copyFrom)) {
            Open5eFields.traitTags.removeFrom(copyFrom);
        }
    ...
    JsonNode mergeNodes(Open5eIndexType type, String originKey, JsonNode copyFrom, JsonNode copyTo) {
        ...
        if (_trait != null) {
            ...
            String templateKey = Open5eIndexType.monsterTemplate.createKey(_trait);
    ...
    private void doScalarMultXp(String originKey, JsonNode modInfo, ObjectNode target) {
        JsonNode crNode = Open5eFields.cr.getFrom(target);
        if (crNode == null) {
        ...
        if (Open5eFields.xp.existsIn(crNode)) {
            double input = Open5eFields.xp.getFrom(crNode).asDouble();
            Open5eFields.xp.setIn(target, new DoubleNode(scalarMult.applyAsDouble(input)));
        } else {
            double crValue = crToXp(crNode);
            if (!Open5eFields.cr.existsIn(crNode)) {
                // Wrap cr in a containing object
                ObjectNode crNodeRw = mapper().createObjectNode();
                Open5eFields.cr.setIn(crNodeRw, crNode);
                Open5eFields.cr.setIn(target, crNodeRw);
                crNode = crNodeRw;
            }
            Open5eFields.xp.setIn(crNode, new DoubleNode(scalarMult.applyAsDouble(crValue)));
        }
    }

    private void doMaxSize(String originKey, JsonNode modInfo, ObjectNode target) {
        final String SIZES = "FDTSMLHGCV";
        if (!Open5eFields.size.existsIn(target)) {
        ...
        ArrayNode size = Open5eFields.size.ensureArrayIn(target);
    ...
    void doAddSkills(String originKey, JsonNode modInfo, ObjectNode target) {
        ...
        int pb = crToPb(Open5eFields.cr.getFrom(target));
