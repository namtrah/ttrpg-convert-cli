...
import dev.ebullient.convert.tools.open5e.Open5eIndex.Tuple;
import dev.ebullient.convert.tools.open5e.qute.AcHp;
import dev.ebullient.convert.tools.open5e.qute.Open5eQuteBase;
import dev.ebullient.convert.tools.open5e.qute.QuteMonster;
import dev.ebullient.convert.tools.open5e.qute.QuteMonster.SavesAndSkills;
import dev.ebullient.convert.tools.open5e.qute.QuteMonster.Spellcasting;
import dev.ebullient.convert.tools.open5e.qute.QuteMonster.Spells;
...
public class Json2QuteMonster extends Json2QuteCommon {
    ...
    Json2QuteMonster(Open5eIndex index, Open5eIndexType type, JsonNode jsonNode) {
    ...
    protected Open5eQuteBase buildQuteResource() {
        ...
        String fluff = getFluffDescription(Open5eIndexType.monsterFluff, "##", fluffImages);
        ...
        if (MonsterFields.legendaryGroup.existsIn(rootNode)) {
            ...
            String lgKey = index().getAliasOrDefault(Open5eIndexType.legendaryGroup.createKey(nameSource));
            if (index.sourceIncluded(SourceField.source.getTextOrThrow(nameSource))) {
                ...
                if (lgNode == null) {
                    ...
                    Open5eSources lgSources = Open5eSources.findSources(lgKey);
                    legendaryGroupText = legendaryGroup(lgNode);
                    legendaryGroupLink = linkifyType(Open5eIndexType.legendaryGroup,
                            lgKey,
                            lgSources.getName());
                }
            } else {
    ...
    void findCreatureType() {
        JsonNode typeNode = type == Open5eIndexType.monster
    ...
    public String getImagePath() {
        ...
        return Open5eQuteBase.monsterPath(isNpc, creatureType);
    }

    public static List<Tuple> findMonsterVariants(Open5eIndex index, Open5eIndexType type,
    ...
    public static List<Tuple> findConjuredMonsterVariants(Open5eIndex index, Open5eIndexType type,
            String key, JsonNode jsonSource) {
    ...
    static void createVariant(Open5eIndex index, List<Tuple> variants,
            JsonNode jsonSource, Open5eIndexType type,
