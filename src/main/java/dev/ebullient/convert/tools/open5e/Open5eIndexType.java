...
import dev.ebullient.convert.tools.open5e.Json2QuteDeck.DeckFields;
import dev.ebullient.convert.tools.open5e.JsonSource.Open5eFields;

public enum Open5eIndexType implements IndexType, JsonNodeReader {
    action,
    adventure,
    adventureData,
    background,
    ...
    Open5eIndexType(String templateName) {
    ...
    public static Open5eIndexType fromText(String name) {
    ...
    public static Open5eIndexType getTypeFromKey(String key) {
        String typeKey = key.substring(0, key.indexOf("|"));
        return fromText(typeKey);
    }

    public static Open5eIndexType getTypeFromNode(JsonNode node) {
    ...
    public String createKey(JsonNode x) {
        if (this == book || this == adventure || this == bookData || this == adventureData) {
            ...
            return createKey(
                    Open5eFields.appliesTo.getTextOrEmpty(x),
    ...
    public String linkify(JsonSource convert, JsonNode entry) {
        ...
        return switch (this) {
            case subclass -> convert.linkify(this, Open5eIndexType.getSubclassTextReference(
                    Open5eFields.className.getTextOrEmpty(entry),
                    Open5eFields.classSource.getTextOrEmpty(entry),
                    name, source, name));
    ...
    public String decoratedName(String name, JsonNode entry) {
        Open5eSources sources = Open5eSources.findOrTemporary(entry);
    ...
    public static String getSubclassFeatureKey(String name, String featureSource, String className, String classSource,
        ...
        return String.format("%s|%s|%s|%s|%s|%s|%s%s",
                Open5eIndexType.subclassFeature,
                name,
                className,
                "phb".equalsIgnoreCase(classSource) ? "" : classSource,
                scShortName,
    ...
    public String vaultRoot(Open5eIndex index) {
    ...
    public void withArrayFrom(JsonNode node, BiConsumer<Open5eIndexType, JsonNode> callback) {
        if (node.has(this.nodeName())) {
            node.withArray(this.nodeName()).forEach(x -> callback.accept(this, x));
        }
    }

    public void withArrayFrom(JsonNode node, String field, BiConsumer<Open5eIndexType, JsonNode> callback) {
        if (node.has(field)) {
