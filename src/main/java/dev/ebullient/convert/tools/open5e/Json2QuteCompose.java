...
public class Json2QuteCompose extends Json2QuteCommon {
    ...
    Open5eSources currentSources;
    final String title;
    final String targetPath;

    public Json2QuteCompose(Open5eIndexType type, Open5eIndex index, String title) {
        this(type, index, title, ".");
    }

    public Json2QuteCompose(Open5eIndexType type, Open5eIndex index, String title, String targetPath) {
    ...
    protected Open5eQuteNote buildQuteNote() {
        ...
        if (type == Open5eIndexType.itemType || type == Open5eIndexType.itemProperty) {
            nodes.forEach(x -> flatten(x));
        }

        nodes.sort(Comparator.comparing(SourceField.name::getTextOrEmpty));

        if (type == Open5eIndexType.itemProperty) {
        ...
        return new Open5eQuteNote(title, null, text, tags)
                .withTargetPath(targetPath);
    }

    private void appendElement(JsonNode entry, List<String> text, Tags tags) {
        currentSources = Open5eSources.findOrTemporary(entry);

        boolean pushed = parseState().push(entry);
        try {
            String abbreviation = Open5eFields.abbreviation.getTextOrNull(entry);
            ...
            if (type == Open5eIndexType.action) {
            ...
            if (type == Open5eIndexType.itemType && abbreviation != null) {
                List<JsonNode> more = index.elementsMatching(Open5eIndexType.itemTypeAdditionalEntries, abbreviation);
                for (JsonNode m : more) {
                    appendToText(text, m, "###");
                }
            }
    ...
    private void flatten(JsonNode entry) {
        ...
        JsonNode next = SourceField.entries.existsIn(entry)
                ...
                : Open5eFields.entriesTemplate.getFrom(entry);
        if (SourceField.name.existsIn(entry) && !SourceField.entries.existsIn(entry)) {
        ...
        tui().warnf("Unable to flatten entry from %s. Content may be missing. %s",
                Open5eSources.findSources(entry), entry);
    }
    ...
    private void appendAction(JsonNode entry, List<String> text) {
        ...
        List<String> seeAlso = ComposedTypeFields.seeAlsoAction.linkifyListFrom(entry, Open5eIndexType.action, index);
        if (!seeAlso.isEmpty()) {
            maybeAddBlankLine(text);
    ...
    private void appendItemProperties(List<String> text, Tags tags) {
        ...
        for (JsonNode srdEntry : iterableElements(srdEntries)) {
            currentSources = Open5eSources.findOrTemporary(srdEntry);
    ...
    private String toAbbv(JsonNode x) {
        return Open5eFields.abbreviation.getTextOrDefault(x,
