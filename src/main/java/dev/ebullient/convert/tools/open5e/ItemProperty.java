...
public interface ItemProperty {
    ...
    String getMarkdownLink(Open5eIndex index);
    ...
    class CustomItemProperty implements ItemProperty {
        ...
        public CustomItemProperty(JsonNode property) {
            ...
            if (name == null) {
                JsonNode entries = SourceField.entries.existsIn(property)
                        ...
                        : Open5eFields.entriesTemplate.getFrom(property);
        ...
        @Override
        public String getMarkdownLink(Open5eIndex index) {
            return propertyToLink.computeIfAbsent(this, p -> {
                List<JsonNode> targets = index.elementsMatching(Open5eIndexType.itemProperty, abbreviation.toLowerCase());
                if (targets.isEmpty() || targets.size() > 1) {
                    return name;
                }
                String key = Open5eIndexType.itemProperty.createKey(targets.get(0));
    ...
    enum PropertyEnum implements ItemProperty {
        ...
        public String getMarkdownLink(Open5eIndex index) {
            ...
            return propertyToLink.computeIfAbsent(this, p -> {
                List<JsonNode> targets = index.elementsMatching(Open5eIndexType.itemProperty, encodedValue.toLowerCase());
                if (targets.isEmpty() || targets.size() > 1) {
                    return longName;
                }
                String key = Open5eIndexType.itemProperty.createKey(targets.get(0));
