...
import dev.ebullient.convert.tools.open5e.qute.QuteClass;
import dev.ebullient.convert.tools.open5e.qute.QuteSubclass;
...
public class Json2QuteClass extends Json2QuteCommon {
    ...
    Json2QuteClass(Open5eIndex index, Open5eIndexType type, JsonNode jsonNode) {
        ...
        try {
            ...
            findClassFeatures(Open5eIndexType.classfeature, jsonNode.get("classFeatures"), classFeatures, "classFeature");
            findSubclasses();
        } finally {
    ...
    protected QuteClass buildQuteResource() {
        ...
        List<String> text = getFluff(Open5eIndexType.classFluff, "##", new ArrayList<>());
    ...
    public List<QuteSubclass> buildSubclasses() {
        ...
        for (Subclass sc : subclasses) {
            ...
            filename = Open5eQuteBase.fixFileName(sc.getName(), sc.sources);
            try {
                ...
                quteSc.add(new QuteSubclass(sc.sources,
                        ...
                        String.format("[%s](%s.md)", decoratedClassName,
                                Open5eQuteBase.getClassResource(getName(), sc.parentClassSource)),
    ...
    void findClassFeatures(Open5eIndexType type, JsonNode arrayElement, List<ClassFeature> features, String fieldName) {
    ...
    static ClassFeature findClassFeature(JsonSource converter, Open5eIndexType type, JsonNode cf, String fieldName) {
        ...
        if (feature == null) {
            ...
            feature.cfSources = Open5eSources.findSources(finalKey);
    ...
    void findSubclasses() {
        ...
        for (JsonNode x : index().classElementsMatching(Open5eIndexType.subclass, getSources().getName(), classSource)) {
        ...
        for (String aliasKey : index.getAliasesTo(getSources().getKey())) {
            ...
            for (JsonNode x : index().classElementsMatching(Open5eIndexType.subclass, getSources().getName(), aliasSource)) {
        ...
        for (JsonNode scNode : scNodes.keySet()) {
            ...
            String scKey = Open5eIndexType.subclass.createKey(scNode);
            ...
            sc.sources = Open5eSources.findSources(scKey);
            ...
            findClassFeatures(Open5eIndexType.subclassFeature, resolved.get("subclassFeatures"), sc.classFeatures, null);
    ...
    void addOptionalFeatureText(JsonNode entry, List<String> text) {
        ...
        String relativePath = Open5eIndexType.optionalFeatureTypes.getRelativePath();
    ...
    String classSkills(JsonNode source, Open5eSources sources) {
    ...
    static class ClassFeature {
        ...
        Open5eSources cfSources;
        Open5eIndexType cfType;
