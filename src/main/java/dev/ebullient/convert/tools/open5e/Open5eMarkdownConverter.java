...
public class Open5eMarkdownConverter implements MarkdownConverter {
    final Open5eIndex index;
    final MarkdownWriter writer;

    public Open5eMarkdownConverter(Open5eIndex index, MarkdownWriter writer) {
        this.index = index;
        this.writer = writer;
    }

    public Open5eMarkdownConverter writeAll() {
        return writeFiles(Stream.of(Open5eIndexType.values())
                .filter(Open5eIndexType::writeFile)
                .collect(Collectors.toList()));
    }

    public Open5eMarkdownConverter writeNotesAndTables() {
        return writeFiles(Stream.of(Open5eIndexType.values())
                .filter(x -> !x.writeFile())
                .filter(Open5eIndexType::useQuteNote)
                .collect(Collectors.toList()));
    }

    public Open5eMarkdownConverter writeImages() {
        index.tui().copyImages(Open5eSources.getImages());
        index.tui().copyFonts(Open5eSources.getFonts());
        return this;
    }

    public Open5eMarkdownConverter writeFiles(IndexType type) {
        return writeFiles(List.of(type));
    }

    public Open5eMarkdownConverter writeFiles(List<? extends IndexType> types) {
        ...
        if (types != null) {
            writeQuteBaseFiles(types.stream()
                    .filter(x -> !((Open5eIndexType) x).useQuteNote())
                    .toList());
    ...
    private void writeQuteBaseFiles(List<? extends IndexType> types) {
        ...
        for (Entry<String, JsonNode> e : index.includedEntries()) {
            ...
            Open5eIndexType nodeType = Open5eIndexType.getTypeFromKey(key);
            if (types.contains(Open5eIndexType.race) && nodeType == Open5eIndexType.subrace) {
                // include subrace with race
            } else if (!types.contains(nodeType)) {
                continue;
            }

            if (nodeType == Open5eIndexType.classtype) {
    ...
    private QuteBase json2qute(Open5eIndexType type, JsonNode jsonSource) {
        return switch (type) {
    ...
    private void writeQuteNoteFiles(List<? extends IndexType> types) {
        final String vrDir = Open5eIndexType.variantrule.getRelativePath();

        List<QuteNote> compendium = new ArrayList<>();
        List<QuteNote> rules = new ArrayList<>();

        Map<Open5eIndexType, Json2QuteCommon> combinedDocs = new HashMap<>();
        ...
        for (Entry<String, JsonNode> e : index.includedEntries()) {
            ...
            switch (nodeType) {
                ...
                case table, tableGroup -> {
                    Open5eQuteNote tableNote = new Json2QuteTable(index, nodeType, node).buildNote();
    ...
    <T extends QuteBase> void append(Open5eIndexType type, T note, List<T> compendium, List<T> rules) {
        if (note != null) {
